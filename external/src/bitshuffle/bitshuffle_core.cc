...
int64_t bshuf_copy(
    const void* in, void* out, const size_t size, const size_t elem_size) {
...
int64_t bshuf_trans_byte_elem_remainder(
    const void* in,
    void* out,
    const size_t size,
...
int64_t bshuf_trans_byte_elem_scal(
    const void* in, void* out, const size_t size, const size_t elem_size) {
  return bshuf_trans_byte_elem_remainder(in, out, size, elem_size, 0);
}

/* Transpose bits within bytes. */
int64_t bshuf_trans_bit_byte_remainder(
    const void* in,
    void* out,
    const size_t size,
    const size_t elem_size,
...
/* Transpose bits within bytes. */
int64_t bshuf_trans_bit_byte_scal(
    const void* in, void* out, const size_t size, const size_t elem_size) {
...
int64_t bshuf_trans_elem(
  ...
  for (ii = 0; ii < lda; ii++) {
    for (jj = 0; jj < ldb; jj++) {
      memcpy(
          &out_b[(jj * lda + ii) * elem_size],
          &in_b[(ii * ldb + jj) * elem_size],
          elem_size);
    }
  }
...
int64_t bshuf_trans_bitrow_eight(
    const void* in, void* out, const size_t size, const size_t elem_size) {
...
int64_t bshuf_trans_bit_elem_scal(
    const void* in, void* out, const size_t size, const size_t elem_size) {
  int64_t count;
  void* tmp_buf;
 ...
 * the bytes. */
int64_t bshuf_trans_byte_bitrow_scal(
    const void* in, void* out, const size_t size, const size_t elem_size) {
...
int64_t bshuf_shuffle_bit_eightelem_scal(
    const void* in, void* out, const size_t size, const size_t elem_size) {
  const char* in_b;
  char* out_b;
  uint64_t x, t;
...
int64_t bshuf_untrans_bit_elem_scal(
    const void* in, void* out, const size_t size, const size_t elem_size) {
  int64_t count;
  void* tmp_buf;
...
int64_t bshuf_trans_byte_elem_SSE_16(
    const void* in, void* out, const size_t size) {
  size_t ii;
  const char* in_b = (const char*)in;
  char* out_b = (char*)out;
  __m128i a0, b0, a1, b1;
...
int64_t bshuf_trans_byte_elem_SSE_32(
    const void* in, void* out, const size_t size) {
  size_t ii;
  const char* in_b;
  char* out_b;
...
int64_t bshuf_trans_byte_elem_SSE_64(
    const void* in, void* out, const size_t size) {
  size_t ii;
  const char* in_b = (const char*)in;
  char* out_b = (char*)out;
...
int64_t bshuf_trans_byte_elem_SSE(
    const void* in, void* out, const size_t size, const size_t elem_size) {
  int64_t count;
...
/* Transpose bits within bytes. */
int64_t bshuf_trans_bit_byte_SSE(
    const void* in, void* out, const size_t size, const size_t elem_size) {
  size_t ii, kk;
  const char* in_b = (const char*)in;
  char* out_b = (char*)out;
  ...
  count = bshuf_trans_bit_byte_remainder(
      in, out, size, elem_size, nbyte - nbyte % 16);
  return count;
}
...
/* For data organized into a row for each bit (8 * elem_size rows), transpose
 * the bytes. */
int64_t bshuf_trans_byte_bitrow_SSE(
    const void* in, void* out, const size_t size, const size_t elem_size) {
  size_t ii, jj;
  const char* in_b = (const char*)in;
  char* out_b = (char*)out;
...
/* Shuffle bits within the bytes of eight element blocks. */
int64_t bshuf_shuffle_bit_eightelem_SSE(
    const void* in, void* out, const size_t size, const size_t elem_size) {
  CHECK_MULT_EIGHT(size);
...
/* Transpose bits within bytes. */
int64_t bshuf_trans_bit_byte_AVX(
    const void* in, void* out, const size_t size, const size_t elem_size) {
  size_t ii, kk;
  const char* in_b = (const char*)in;
  char* out_b = (char*)out;
  ...
  count = bshuf_trans_bit_byte_remainder(
      in, out, size, elem_size, nbyte - nbyte % 32);
  return count;
}

/* Transpose bits within elements. */
int64_t bshuf_trans_bit_elem_AVX(
    const void* in, void* out, const size_t size, const size_t elem_size) {
  int64_t count;
...
/* For data organized into a row for each bit (8 * elem_size rows), transpose
 * the bytes. */
int64_t bshuf_trans_byte_bitrow_AVX(
    const void* in, void* out, const size_t size, const size_t elem_size) {
...
/* Shuffle bits within the bytes of eight element blocks. */
int64_t bshuf_shuffle_bit_eightelem_AVX(
    const void* in, void* out, const size_t size, const size_t elem_size) {
  CHECK_MULT_EIGHT(size);
...
int64_t bshuf_untrans_bit_elem(
    const void* in, void* out, const size_t size, const size_t elem_size) {
  int64_t count;
#ifdef USEAVX2
 ...
 * parallel. */
int64_t bshuf_blocked_wrap_fun(
    bshufBlockFunDef fun,
    const void* in,
    void* out,
    const size_t size,
    const size_t elem_size,
...
int64_t bshuf_bitshuffle_block(
    ioc_chain* C_ptr, const size_t size, const size_t elem_size) {
  size_t this_iter;
  const void* in;
  void* out;
  int64_t count;
...
int64_t bshuf_bitunshuffle_block(
    ioc_chain* C_ptr, const size_t size, const size_t elem_size) {
  size_t this_iter;
  const void* in;
  void* out;
...
int64_t bshuf_bitshuffle(
    ...
    const size_t elem_size,
    size_t block_size) {
  return bshuf_blocked_wrap_fun(
      &bshuf_bitshuffle_block, in, out, size, elem_size, block_size);
}
...
int64_t bshuf_bitunshuffle(
    ...
    size_t block_size) {
  return bshuf_blocked_wrap_fun(
      &bshuf_bitunshuffle_block, in, out, size, elem_size, block_size);
}
