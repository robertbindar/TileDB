...
typedef void (*shuffle_func)(
    const size_t, const size_t, const uint8_t*, const uint8_t*);
typedef void (*unshuffle_func)(
    const size_t, const size_t, const uint8_t*, const uint8_t*);
typedef int64_t (*bitshuffle_func)(
    void*, void*, const size_t, const size_t, void*);
typedef int64_t (*bitunshuffle_func)(
    void*, void*, const size_t, const size_t, void*);
...
#if defined(SHUFFLE_AVX2_ENABLED) || \
    defined(SHUFFLE_SSE2_ENABLED) /* Intel/i686 */
...
__attribute__((always_inline)) static inline void blosc_internal_cpuidex(
    int32_t cpuInfo[4], int32_t function_id, int32_t subfunction_id) {
  __asm__ __volatile__(
#if defined(__i386__) && defined(__PIC__)
      /*  Can't clobber ebx with PIC running under 32-bit, so it needs to be
         manually restored.
...
#define blosc_internal_cpuid(cpuInfo, function_id) \
  blosc_internal_cpuidex(cpuInfo, function_id, 0)
...
/* Reads the content of an extended control register.
   https://software.intel.com/en-us/articles/how-to-detect-new-instruction-support-in-the-4th-generation-intel-core-processor-family
*/
static inline uint64_t blosc_internal_xgetbv(uint32_t xcr) {
  uint32_t eax, edx;
  __asm__ __volatile__(
      /* "xgetbv"
...
static blosc_cpu_features blosc_get_cpu_features(void) {
  ...
  /*  Even if certain features are supported by the CPU, they may not be
     supported by the OS (in which case using them would crash the process or
     system). If xsave is available and enabled by the OS, check the contents of
     the
      extended control register XCR0 to see if the CPU features are enabled. */
#if defined(_XCR_XFEATURE_ENABLED_MASK)
  if (xsave_available && xsave_enabled_by_os &&
      (sse2_available || sse3_available || ssse3_available || sse41_available ||
       sse42_available || avx2_available || avx512bw_available)) {
...
#if defined(_MSC_VER)
#pragma message( \
    "Hardware-acceleration detection not implemented for the target architecture. Only the generic shuffle/unshuffle routines will be available.")
#else
...
static shuffle_implementation_t get_shuffle_implementation(void) {
  ...
  if (cpu_features & BLOSC_HAVE_AVX2) {
    ...
    impl_avx2.bitshuffle =
        (bitshuffle_func)blosc_internal_bshuf_trans_bit_elem_avx2;
    impl_avx2.bitunshuffle =
        (bitunshuffle_func)blosc_internal_bshuf_untrans_bit_elem_avx2;
    return impl_avx2;
  }
#endif /* defined(SHUFFLE_AVX2_ENABLED) */
  ...
  if (cpu_features & BLOSC_HAVE_SSE2) {
    ...
    impl_sse2.bitshuffle =
        (bitshuffle_func)blosc_internal_bshuf_trans_bit_elem_sse2;
    impl_sse2.bitunshuffle =
        (bitunshuffle_func)blosc_internal_bshuf_untrans_bit_elem_sse2;
    return impl_sse2;
  }
#endif /* defined(SHUFFLE_SSE2_ENABLED) */
  ...
  impl_generic.bitshuffle =
      (bitshuffle_func)blosc_internal_bshuf_trans_bit_elem_scal;
  impl_generic.bitunshuffle =
      (bitunshuffle_func)blosc_internal_bshuf_untrans_bit_elem_scal;
  return impl_generic;
}
...
/*  Bit-unshuffle a block by dynamically dispatching to the appropriate
    hardware-accelerated routine at run-time. */
int blosc_internal_bitunshuffle(
    const size_t bytesoftype,
    const size_t blocksize,
    const uint8_t* const _src,
    const uint8_t* _dest,
