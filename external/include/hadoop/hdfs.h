...
hdfsFS hdfsConnectAsUserNewInstance(
    const char* nn, tPort port, const char* user);
...
void hdfsBuilderSetKerbTicketCachePath(
    struct hdfsBuilder* bld, const char* kerbTicketCachePath);
...
int hdfsBuilderConfSetStr(
    struct hdfsBuilder* bld, const char* key, const char* val);
 ...
 */
LIBHDFS_EXTERNAL
struct hdfsStreamBuilder* hdfsStreamBuilderAlloc(
    hdfsFS fs, const char* path, int flags);
...
int hdfsStreamBuilderSetBufferSize(
    struct hdfsStreamBuilder* bld, int32_t bufferSize);
...
int hdfsStreamBuilderSetReplication(
    struct hdfsStreamBuilder* bld, int16_t replication);
...
int hdfsStreamBuilderSetDefaultBlockSize(
    struct hdfsStreamBuilder* bld, int64_t defaultBlockSize);
 ...
 */
LIBHDFS_EXTERNAL
struct hadoopRzBuffer* hadoopReadZero(
    hdfsFile file, struct hadoopRzOptions* opts, int32_t maxLength);
