  ...
  virtual pos_type seekoff(
    ...
    if (!in && !out)
      return pos_type(off_type(-1));
        ...
        (in &&
         (!(m_mode & std::ios_base::in) || (off != 0 && this->gptr() == 0))) ||
        (out &&
         (!(m_mode & std::ios_base::out) || (off != 0 && this->pptr() == 0))))
      return pos_type(off_type(-1));
    ...
    switch (dir) {
      ...
      case std::ios_base::cur:
        newoff = in ?
                     static_cast<std::streamoff>(this->gptr() - this->eback()) :
...
class basic_ibufferstream :
 ...
 public:  // Typedefs
  ...
  typedef
      typename std::basic_ios<char_type, CharTraits>::traits_type traits_type;
 ...
 public:
  ...
  basic_ibufferstream(std::ios_base::openmode mode = std::ios_base::in)
      :  // basic_ios_t() is called first (lefting it uninitialized) as it's a
         ...
         // calling the basic_ios_t's destructor without calling
         // basic_ios_t::init()
